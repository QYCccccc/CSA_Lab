...
int s = 0; /* set index bits 组索引位数*/
int b = 0; /* block offset bits 块内偏移位数*/
int E = 0; /* associativity 每组包含的缓存行数*/
...
void updateLruNum(cache_set_t cache_set, int hit_index) {
    cache_set[hit_index].lru = 0;
    for(int i = 0; i < E; i++)
        cache_set[i].lru++;        //命中清0，其余都+1
}
/* 
  ...
  bool isMiss = true;
    ...
    if(isMiss) {
        ...
        if(isFull) {
            ...
            //更新cache数据
            cache_set[evi_index].valid = 1;
            cache_set[evi_index].tag = tag;
            eviction_count++;   //淘汰计数+1
            updateLruNum(cache_set, evi_index);
